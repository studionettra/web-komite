...
import { CaretRight, MagnifyingGlass, Tag, Newspaper, Megaphone, ChatCircle, BellRinging } from '@phosphor-icons/react';
    ...
    return (
        <PublicLayout>
            <Head>
                ...
                <meta name="description" content="Kumpulan berita, artikel, dan kabar terbaru dari kegiatan Komite KBIT-TKIT Al-Ikhlash Pasar Minggu." />
            </Head>
            ...
            <section className="relative z-0 overflow-hidden border-b-[6px] border-dashed border-sky-200 bg-sky-50 pt-28 pb-20 text-center sm:pt-32 sm:pb-32">
                ...
                <div className="relative z-10 mx-auto max-w-4xl px-4 text-center sm:px-6 lg:px-8">
                    <div className="inline-block rounded-[2.5rem] border border-white/60 bg-white/70 p-6 shadow-xl shadow-sky-900/5 backdrop-blur-xl sm:p-10">
                        <span className="inline-flex items-center gap-2 rounded-full bg-blue-50 px-4 py-1.5 text-xs font-extrabold text-blue-600 ring-1 ring-blue-500/20 ring-inset mb-4 sm:mb-6">
                        ...
                        <h1 className="mb-4 text-3xl font-extrabold tracking-tight text-slate-800 sm:mb-6 sm:text-5xl">
                            Berita & <span className="text-blue-600">Informasi</span>
                        </h1>
            ...
            <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12">
                <div className="flex flex-col lg:flex-row gap-8">
                    ...
                    <div className="lg:w-3/4">
                        {posts.data.length === 0 ? (
                            <div className="flex flex-col items-center justify-center rounded-[2.5rem] bg-white p-16 text-center shadow-[0_8px_30px_rgba(0,0,0,0.04)] border border-slate-100">
                            ...
                            <>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                    {posts.data.map((post: any, index: number) => (
                                        <article 
                                            key={post.id} 
                                            className={`group flex flex-col overflow-hidden rounded-[2.5rem] bg-white shadow-[0_8px_30px_rgba(0,0,0,0.04)] border border-slate-100 transition-all hover:shadow-[0_20px_40px_rgba(0,0,0,0.08)] hover:-translate-y-1 ${index === 0 ? 'md:col-span-2 md:flex-row' : ''}`}
                                        >
                                            <div className={`relative overflow-hidden bg-slate-100 ${index === 0 ? 'md:w-1/2 md:shrink-0' : 'aspect-video'}`}>
                                                {post.image_path ? (
                                                    <img 
                                                    ...
                                                    <div className="flex h-full w-full items-center justify-center bg-slate-100">
                                                        <Newspaper weight="duotone" className="h-16 w-16 text-slate-300" />
                                                    </div>
                                                )}
                                                ...
                                                {post.category && (
                                                    <div className="absolute top-4 left-4">
                                                        <span 
                                                            ...
                                                            style={{ 
                                                                backgroundColor: post.category.color ? `${post.category.color}e6` : 'rgba(59, 130, 246, 0.9)', 
                                                                color: '#fff' 
                                                            }}
                                                        >
                                                            {post.category.name}
                                            ...
                                            <div className={`flex flex-1 flex-col p-8 ${index === 0 ? 'md:justify-center' : ''}`}>
                                                ...
                                                <h2 className={`font-extrabold text-slate-900 mb-4 transition-colors group-hover:text-blue-600 line-clamp-3 ${index === 0 ? 'text-2xl md:text-3xl' : 'text-xl'}`}>
                                                    <Link href={`/kabar/${post.slug}`}>
                                                        {post.title}
                                                    </Link>
                                                </h2>
                                                ...
                                                <div 
                                                    className="text-slate-600 font-medium leading-relaxed line-clamp-3 mb-6"
                    ...
                    <div className="lg:w-1/4">
                        <div className="sticky top-28 space-y-8">
                            <div className="rounded-[2rem] bg-white p-6 shadow-[0_8px_30px_rgba(0,0,0,0.04)] border border-slate-100">
                                <h3 className="text-lg font-extrabold text-slate-800 mb-4 border-b border-slate-100 pb-4">
                                    Kategori
                                </h3>
